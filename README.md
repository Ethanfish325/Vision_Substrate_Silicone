...
### 2. 核心视觉工具（工具箱精简版，2026-09 起）

> 针对"导热胶垫 + SN 条码"检测场景，工具箱精简为下列算子，配置更快；
> 图像处理/几何/测量等旧算子类与代码仍保留（旧方案文件兼容加载），仅不再出现在工具箱。

| 类别 | 工具 | 功能 |
|------|------|------|
| 定位 | `PositionCorrect` | 位置修正：在参考图框选稳定特征为基准，运行时模板匹配求平移/180°翻转/小角度偏移，整图校正回参考姿态，固定 ROI 自动随动（解决产品偏移误判 NG） |
| 区域 | `MultiROI` | 多区域 ROI：命名区域、百分比坐标、启用/禁用 |
| 识别 | `QRCodeRecognize` | 条码识别：二维码 + 一维码，结果作为板卡 SN（多策略解码：两种极性/多种自适应核/多尺度；大 ROI 自动"粗定位条码区域 + 局部放大"；带时间预算） |
| 识别 | `ColorRecognition` | 颜色识别（胶垫检测）：专属取色界面 + 覆盖率上下限判定 |

### （历史）六大类视觉工具（34 种，代码保留、工具箱不再显示）
...
2. 为每个位置关联视觉方案（可选 位置修正 → MultiROI + QRCodeRecognize + ColorRecognition）
3. 切换到自动化模式，选择产品
4. 点击「启动监听」后，通过 DI 触发（启动按钮）或手动触发执行检测
5. 系统自动：运动到各点位 → 拍照 → （位置修正）→ 检测 → 拼接 → 识别 QR（SN）
...
1. 创建或打开检测方案
2. 从算子工具箱拖拽算子到流水线插槽（工具箱已精简为核心算子）
3. 点击算子配置参数（支持实时预览）
4. 加载测试图像进行预览
5. 保存方案

### 4.1 位置修正（PositionCorrect）使用步骤

> 目的：产品摆放平移/180°翻转/小角度歪斜时，固定 ROI 不再误判 NG。

1. 让设备停在**标准摆放位**，用相机拍一张"参考图"（或加载标准样板图）；
2. 从工具箱把「位置修正」拖到流水线**最前面**（第一步），双击它打开配置；
3. 点「载入参考图」，在产品上框选一个**稳定且独特**的特征（板角 L 形、丝印、mark 等；框略大于特征）；
4. 选择旋转容差：无翻转选"仅0°"，可能反放选"0°/180°翻转"，小角度歪斜选"小角度范围"并设范围/步长；
5. 点「试运行」确认能定位（score≈1、角度≈0），确定保存；
6. 后续照常拖入 MultiROI 等，ROI 按参考姿态框选即可——运行时先自动校正再检测。

### 4.2 胶垫颜色检测（ColorRecognition）使用步骤

1. 先用 MultiROI 框出胶垫区域；
2. 拖入「颜色识别」，在参数窗点「🎨 取色」→ 在图上**点选/框选胶色**，自动生成颜色模型；
3. 设置覆盖率合格范围（下限防缺胶、上限防溢胶），观察实时预览与占比；
4. 保存方案。
