# NMC1400 四轴运动控制卡轴控制功能实施计划

## 一、目标

在现有视觉检测系统中集成 NMC1400 四轴运动控制卡，实现：

1. **软件启动自动连接** NMC1400（网口通信，IP 默认 `192.168.1.11`，写死在代码中）
...
| 文件 | 状态 | 说明 |
|------|------|------|
| [`core/smcsh_dll.py`](../core/smcsh_dll.py) | ✅ 完整（614行） | NMC1400 DLL 封装：连接、运动参数、点位运动、定速运动、停止/状态/位置、回零 |
