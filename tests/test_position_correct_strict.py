# -*- coding: utf-8 -*-
"""位置修正严格自测(ROI 随动语义):小角度旋转方向、大模板缩放路径、
location 几何正确性、流水线联动。

架构说明:位置修正不旋转整图(processed_image = 原始输入),只写
context.location。校验通过"按 location 在原图上还原模板区域内容"进行:
若 ROI 随动正确,摆正裁剪出的区域应当就是模板本身。
"""
import sys
...
def region_content_diff(img_bgr, loc, templ_bgr):
    """按 location 摆正还原模板区域,返回与模板的灰度平均差(ROI 随动证据)。"""
...
def test_positive_angle():
    """参考图逆时针旋转 +8°,位置修正应报告角度≈+8,location 能随动。"""
    print("\n[1] +8° 小角度旋转(逆时针)")
    ref = make_ref_image()
    ...
    res, loc = run_and_locate(tool, rotated)
    ...
    if res.processed_image is not None:
        raw_diff = cv2.absdiff(res.processed_image, rotated).mean()
        check("processed_image = 原始输入(整图不旋转)", raw_diff < 1,
              f"mean_diff={raw_diff:.2f}")
    if loc:
        diff = region_content_diff(rotated, loc, tool._get_template())
        check("按 location 还原模板区≈模板(随动摆正正确)", diff < 12,
              f"mean_diff={diff:.2f}")
...
def test_large_template_scale():
    ...
    现场配置问题而非逻辑错误;此处把阈值放宽以专门验证几何换算。缩放匹配
    的锚点有 ~5px 量级近似误差(可接受),故平移量容差放宽。
    """
    ...
    d = res.data
    check("dx≈60", d.get("matched") and abs(d["dx"] - 60) <= 12,
          f"dx={d.get('dx')}")
    check("dy≈40", d.get("matched") and abs(d["dy"] - 40) <= 12,
          f"dy={d.get('dy')}")
    if res.processed_image is not None:
        ...
        check("processed_image = 原始输入(整图不旋转)", raw_diff < 1,
              f"mean_diff={raw_diff:.2f}")
    if loc:
        check("location 已写入(随动可用)", loc.get("matched") is True,
              f"loc={loc}")
...
def test_shift_pixel_exact():
    ...
    if res.processed_image is not None:
        ...
        check("processed_image = 原始输入(整图不旋转)", raw_diff < 1,
              f"mean_diff={raw_diff:.2f}")
    if loc:
        diff = region_content_diff(moved, loc, tool._get_template())
        check("按 location 还原模板区≈模板", diff < 10,
              f"mean_diff={diff:.2f}")


def test_flip_pipeline():
    """180° 翻转 + 胶垫颜色检测联动(整条链路,ROI 随动)。胶垫避开翻转对称中心。"""
    print("\n[5] 翻转场景流水线联动(ROI 随动)")
    ref = make_ref_image()
    ...
    check("翻转后整板仍OK(位置修正把胶垫 ROI 随动到翻转后实际位置)", bool(ok),
          f"failed={[r.message for r in results if not bool(r.passed)]}")
    # MultiROI 步骤应报告随动(翻转 180 → 实际 ROI 不等于参考坐标)
    roi_data = results[1].data if len(results) > 1 else {}
    followed = roi_data.get("followed")
    check("MultiROI 报告 ROI 随动", bool(followed), f"data={roi_data}")
    # 对照组:无位置修正
    pipe2 = Pipeline("对照")
